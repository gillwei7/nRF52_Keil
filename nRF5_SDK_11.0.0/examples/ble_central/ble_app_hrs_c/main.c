...
/* Marcus, Central Side control the Connection Interval */
#if THROUGHPUT_TEST
...
uint8_t testData[TEST_DATA_SIZE] = {0};
static uint32_t counterStart = 0;
static uint32_t counterStop = 0;
static uint32_t counterPkt = 0;
#endif
...
static void bas_c_evt_handler(ble_bas_c_t * p_bas_c, ble_bas_c_evt_t * p_bas_c_evt)
{
    ...
    switch (p_bas_c_evt->evt_type)
    {
        ...
        case BLE_BAS_C_EVT_BATT_NOTIFICATION:
        {
						...
						if (counterPkt == 0)
						{
								/* Record the first received counter */
								printf("\r\nFirst:%x %x\r\n", testData[0], testData[1]);
								counterStart = NRF_RTC0->COUNTER;
								counterPkt++;
						} else if (counterPkt >= 1000) {
								/* print the start & stop counter */
								counterStop = NRF_RTC0->COUNTER;
								printf("\r\ncounterStart:%d counterStop:%d End:%x %x\r\n"
											, counterStart, counterStop, testData[0], testData[1]);
								counterStart = 0;
								counterStop = 0;
								counterPkt = 0;
						} else {
