...
/* Marcus add, used to test data */
static uint16_t dummyCount = 0;
...
/* Marcus add, used to test data */
		attr_char_value.max_len   = TEST_DATA_SIZE * sizeof(uint8_t);
...
uint32_t ble_bas_battery_level_update(ble_bas_t * p_bas, uint8_t battery_level)
{
    ...
    ble_gatts_value_t gatts_value;
		...
		/* Marcus add, used to test data */
		uint8_t dummyData[TEST_DATA_SIZE] = {0x00,0x00,0x03,0x04,0x05,
																				 0x06,0x07,0x08,0x09,0x0A,
																				 0x0B,0x0C,0x0D,0x0E,0x0F,
																				 0x00,0x01,0x02,0x03,0x04};
		dummyData[0] = dummyCount & 0x000000FF;
		dummyData[1] = (dummyCount >> 8) & 0x000000FF;
		
		if (dummyCount < 0xFFFF)
			dummyCount++;
		else
			dummyCount = 0;
...
*/
        gatts_value.len     = sizeof(dummyData);
        gatts_value.offset  = 0;
        gatts_value.p_value = dummyData;
